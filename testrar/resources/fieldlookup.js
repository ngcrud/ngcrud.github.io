...
function ConstructLookup(requestUrl, inputFieldId, dialogTarget, dialogOptionalTarget, formName, width, height, position, modal, ajaxUrl, showDescription, presentation, defaultMinLength, defaultDelay, args) {
    ...
    var dialogOpts = {
        ...
        open: function(event,ui) {
            ...
            // Added by Motalib for adding extra parameters to show in lookup form
            
            var existParameterArgs = document.getElementById(formName + '_LookupParametersValueField') != null;
            

            if (existParameterArgs){
            	
	            var parameters_value_field = document.getElementById(formName + '_LookupParametersValueField').value;
	            
	            if (parameters_value_field != null && parameters_value_field != ""){
		            var lookup_parameters = parameters_value_field.split(",");
		            
		            for (var lookup_parameter=0; lookup_parameter < lookup_parameters.length; lookup_parameter++){
		            	queryArgs += "&" + lookup_parameters[lookup_parameter] + "=" + document.getElementById(formName + '_' + lookup_parameters[lookup_parameter]).value;
		            }
	            }
...
//Set Value for Mastar LC

function set_value_For_Slash_Input(value) {
	var valueSplitBySpace=value.split("$");
	var newSubString=valueSplitBySpace[0].substring(valueSplitBySpace[0].indexOf("[")+1, valueSplitBySpace[0].indexOf("]")) ;
	var newValue=newSubString.toString();
	var byte1=newValue.split(","); 
	value1=String.fromCharCode.apply(this,byte1);
	value=value1+valueSplitBySpace[1];
    if(GLOBAL_LOOKUP_REF.getReference(ACTIVATED_LOOKUP)){
        obj_caller.target = GLOBAL_LOOKUP_REF.getReference(ACTIVATED_LOOKUP).target;
    }
    else {
