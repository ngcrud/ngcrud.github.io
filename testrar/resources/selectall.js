...
function ajaxAutoCompleter(areaCsvString, showDescription, defaultMinLength, defaultDelay, formName){
    ...
    for (var i = 0; i < numAreas * 3; i = i + 3) {
        ...
        if(formName == 'AssignIncrementPerson'){
        	url = url + "&emplPositionId="+document.getElementById('AssignIncrementPerson_emplPositionId').value;
        }
...
function submitFormApproval(form) {
    var formName = form.name;
    if(formName=="approvalButton"){
        form.parentNode.elements['approvalIndicator_o_0'].value="Y";
//                alert(form.elements['approvalIndicator_o_0'].value);
    }
}
